import * as technologyService from '../../services/enterprise/technology'
...
export default {
    ...
    effects: {
      *query({ payload:{page} }, { call, put }) {  // eslint-disable-line
        const data =yield call(technologyService.query,page,10);
      ...
      *create({payload:values},{call,put}){
        console.log(values);
        yield call(technologyService.add,values);
        yield put({type:'reload'});
      },
      *delete({payload:{id}},{call,put}){
        yield call(technologyService.remove,{id});
        yield put({type:'reload'});
      },
      *update({payload:technologyid,values},{call,put}){
        yield call(technologyService.update,technologyid,values);
        yield put({type:'reload'});
      },
      *reload(action, { put, select }) {
        const page = yield select(state => state.technology.page)
