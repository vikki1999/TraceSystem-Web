import * as distributeService from '../../services/enterprise/distribute'
...
export default {
    ...
    effects: {
      *query({ payload:{page} }, { call, put }) {  // eslint-disable-line
        const data =yield call(distributeService.query,page,10,companyId);
      ...
      *create({payload:values},{call,put}){
        console.log(values);
        yield call(distributeService.add,values);
        yield put({type:'reload'});
      },
      *delete({payload:{id}},{call,put}){
        yield call(distributeService.remove,{id});
        yield put({type:'reload'});
      },
      *update({payload:values},{call,put}){
        yield call(distributeService.update,values);
        yield put({type:'reload'});
      },
